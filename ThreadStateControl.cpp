        ...
        if(mStates[m_iStateIndex].bInitial)
        {
            ...
            for( string s : mStates[m_iStateIndex].v_str_Action)
            {
                ...
                if( s == "AccumulateConversation" )
                {
                    ...
                    //Use m_iPreviousStateIndex (the state we actually just came from) instead of
                    //m_iStateIndex-1: once branches exist, the previous state is not necessarily
                    //"this index minus one" anymore.
                    if( m_iPreviousStateIndex >= 0 )
                    {
                        mStates[m_iStateIndex].message_history = mStates[m_iPreviousStateIndex].message_history;
                    }
            ...
            if( mbWaitForTTSComplete)
            {
                if( mbTTSComplete)
                {
                            ...
                            (WhisperResult.tSpeechEnd > mtimestamp_TTSComplete && current_time - WhisperResult.tSpeechEnd > 3s) )
                    {
                        ...
                        //---- Branching: if this state defines "Branches" in JSON, decide the
                        //     destination based on which keyword group matched the patient's answer.
                        //     States without a "Branches" array are completely unaffected. ----
                        if( mStates[m_iStateIndex].v_Branches.size() > 0)
                        {
                            bool bBranchMatched = false;
                            for( const BranchCondition& branch : mStates[m_iStateIndex].v_Branches)
                            {
                                for( const string& keyword : branch.v_str_Keywords)
                                {
                                    if( WhisperResult.sOutput.find(keyword) != string::npos)
                                    {
                                        mStates[m_iStateIndex].iNextStateIndexOverride = branch.iTargetStateIndex;
                                        bBranchMatched = true;
                                        break;
                                    }
                                }
                                if( bBranchMatched ) break;
                            }

                            if( !bBranchMatched && mStates[m_iStateIndex].iWrongAnswerStateIndex != -1)
                            {
                                mStates[m_iStateIndex].iNextStateIndexOverride = mStates[m_iStateIndex].iWrongAnswerStateIndex;
                            }

                            //debug
                            cout << "Branch decision for state " << m_iStateIndex << ": next -> "
                                 << (mStates[m_iStateIndex].iNextStateIndexOverride != -1 ?
                                     mStates[m_iStateIndex].iNextStateIndexOverride : mStates[m_iStateIndex].iNextStateIndex)
                                 << (bBranchMatched ? " (matched)" : " (no match / wrong-answer fallback)") << endl;

                            //Once the answer has been checked against the branch conditions, the
                            //robot has enough information to move on.
                            mbReadyToChangeState = true;
                        }
...
//                    mStates[m_iStateIndex].message_history.push_back("Assistant: "+msLLMResult);
            ...
            if(current_time - mStates[m_iStateIndex].m_Start_time > mStates[m_iStateIndex].m_secDurationLimit)
            {
                //This is the safety net: without setting mbOldStateComplete here too, a state
                //that never receives a matching keyword (e.g. total silence) would wait forever --
                //mbReadyToChangeState alone is not enough to trigger a transition.
                if( mStates[m_iStateIndex].v_Branches.size() > 0 && mStates[m_iStateIndex].iNextStateIndexOverride == -1 && mStates[m_iStateIndex].iWrongAnswerStateIndex != -1)
                {
                    //A quiz question that timed out with no answer at all -- treat it the same as a wrong answer.
                    mStates[m_iStateIndex].iNextStateIndexOverride = mStates[m_iStateIndex].iWrongAnswerStateIndex;
                    cout << "State " << m_iStateIndex << " timed out with no answer -> routing to wrong-answer state " << mStates[m_iStateIndex].iWrongAnswerStateIndex << endl;
                }
                else
                {
                    cout << "State " << m_iStateIndex << " timed out -> moving on." << endl;
                }
                mbReadyToChangeState = true;
                mbOldStateComplete = true;
            }
        ...
        if( mbReadyToChangeState && mbOldStateComplete)
        {
            ...
            {
                //If branch-matching decided a destination for this visit, use it; otherwise fall
                //back to the normal iNextStateIndex from JSON. The override is runtime-only and is
                //consumed (reset to -1) right here so it never leaks into a future re-visit of
                //this state (e.g. after Restart()).
                int iTargetStateIndex = mStates[m_iStateIndex].iNextStateIndex;
                if( mStates[m_iStateIndex].iNextStateIndexOverride != -1)
                {
                    iTargetStateIndex = mStates[m_iStateIndex].iNextStateIndexOverride;
                    mStates[m_iStateIndex].iNextStateIndexOverride = -1;
                }

                m_iPreviousStateIndex = m_iStateIndex;
                m_iStateIndex = iTargetStateIndex;
                mbReadyToChangeState = false;
            }
        }
